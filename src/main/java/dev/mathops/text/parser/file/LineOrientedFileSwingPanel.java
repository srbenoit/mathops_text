 ...
 */
public final class LineOrientedFileSwingPanel extends JPanel implements Scrollable {

    /** The source file to present. */
    private final LineOrientedFile sourceFile;
     ...
     */
    public LineOrientedFileSwingPanel(final LineOrientedFile theSourceFile) {
        ...
        this.plainFont = new Font(Font.MONOSPACED, Font.PLAIN, 13);
        this.boldFont = new Font(Font.MONOSPACED, Font.BOLD, 13);
        this.italicFont = new Font(Font.MONOSPACED, Font.ITALIC, 13);
        this.boldItalicFont = new Font(Font.MONOSPACED, Font.BOLD | Font.ITALIC, 13);
        this.charBoxSize = new Dimension(0, 0);
    }

    /**
     * Call this after the panel has been added to a tree of UI components
     */
    public void init() {
        ...
        this.charBoxSize.setSize(metrics.stringWidth("0"), metrics.getHeight());

        // Determine preferred size and layout
        final int numLines = this.sourceFile.getNumLines();
        final int numColumns = this.sourceFile.getMaxColumns();
    ...
    public void paintComponent(final Graphics g) {
        ...
        final List<LineOrientedFileSpan> spans = new ArrayList<>(10);

        final int numLines = this.sourceFile.getNumLines();
        for (int lineIndex = 0; lineIndex < numLines; ++lineIndex) {
            final int top = y - metrics.getAscent();
            final int bottom = top + metrics.getHeight();
            ...
            if (top < (clip.y + clip.height) && bottom > clip.y) {
                ...
                drawLine(g, metrics, line, x, y, lineIndex, spans);
                spans.clear();
            }
     ...
     * @param metrics   the font metrics for the plain font
     ...
     */
    private void drawLine(final Graphics g, final FontMetrics metrics, final String line, final int x, final int y,
                          final int lineIndex, final List<LineOrientedFileSpan> spans) {
        ...
        if (spans.isEmpty()) {
            ...
            final int topY = y - metrics.getAscent();
            final int botY = y + metrics.getDescent();
            final int midY = y - metrics.getAscent() / 3;
            ...
            for (int charIndex = 0; charIndex < numChars; ++charIndex) {
                ...
                for (final LineOrientedFileSpan span : spans) {
